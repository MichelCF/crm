...
def print_recent_sales(limit=5):
    ...
    print("-" * 80)
    print(
        f"{'TRANSACTION':<15} | {'STATUS':<10} | {'PRICE':<10} | {'BUYER NAME':<20} | {'PURCHASED AT'}"
    )
    print("-" * 80)
    ...
    for row in rows:
        print(
            f"{row['transaction_id']:<15} | {row['status']:<10} | R$ {row['total_price']:<7.2f} | {row['buyer_name'][:18]:<20} | {row['purchased_at']}"
        )
