...
from src.pipelines.manychat_csv_importer import excel_date_to_datetime, import_manychat_csv
...
def test_import_manychat_csv_skips_empty_contacts(mock_csv_reader, mock_open, mock_get_conn):
    ...
    insert_raw_calls = [call for call in mock_cur.execute.call_args_list if "INSERT INTO manychat_contacts" in call[0][0]]
    assert len(insert_raw_calls) == 1
    
    # 2. It must NOT query or insert into customers (master)
    query_master_calls = [call for call in mock_cur.execute.call_args_list if "customers" in call[0][0].lower()]
    assert len(query_master_calls) == 0

@patch('src.pipelines.manychat_csv_importer.get_connection')
@patch('builtins.open')
@patch('src.pipelines.manychat_csv_importer.csv.DictReader')
def test_import_manychat_csv_creates_new_master(mock_csv_reader, mock_open, mock_get_conn):
    ...
    select_email_calls = [call for call in mock_cur.execute.call_args_list if "SELECT id FROM customers WHERE master_email" in call[0][0]]
    assert len(select_email_calls) == 1
    
    # It must insert into new master
    insert_master_calls = [call for call in mock_cur.execute.call_args_list if "INSERT INTO customers" in call[0][0]]
    assert len(insert_master_calls) == 1

@patch('src.pipelines.manychat_csv_importer.get_connection')
@patch('builtins.open')
@patch('src.pipelines.manychat_csv_importer.csv.DictReader')
def test_import_manychat_csv_updates_existing_master(mock_csv_reader, mock_open, mock_get_conn):
    ...
    select_phone_calls = [call for call in mock_cur.execute.call_args_list if "SELECT id FROM customers WHERE master_phone" in call[0][0]]
    assert len(select_phone_calls) == 1
    
    # It must UPDATE the master, not insert
    update_master_calls = [call for call in mock_cur.execute.call_args_list if "UPDATE customers" in call[0][0]]
