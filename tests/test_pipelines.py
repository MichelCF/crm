...
def test_fetch_and_save_sales_pagination(mock_upsert_sale, mock_upsert_product, mock_upsert_customer, mock_get_sales):
    """Verifies that the loop fetches multiple pages when next_page_token is present."""
    # Setup mock responses for 2 pages
    page_1 = {
        "items": [{"transaction": "TX1", "status": "APPROVED", "purchase": {"price": {"value": 100}, "order_date": 1704067200000}}],
        "page_info": {"next_page_token": "token_abc123"}
    }
    page_2 = {
        "items": [{"transaction": "TX2", "status": "COMPLETE", "purchase": {"price": {"value": 200}, "order_date": 1704067300000}}],
    ...
    mock_get_sales.assert_any_call(start_date="1000", end_date="2000", page_token="token_abc123")
...
def test_sync_sales_to_db_empty(mock_get_conn, mock_init, mock_inc, mock_init_sync, mock_get_max):
...
@patch("src.pipelines.hotmart_to_db.init_db")
@patch("src.pipelines.hotmart_to_db.get_connection")
def test_sync_sales_to_db_incremental(mock_get_conn, mock_init, mock_inc, mock_init_sync, mock_get_max):
