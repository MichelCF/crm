    ...
    try:
        with open(file_path, mode="r", encoding="utf-8") as file:
            ...
            for row in reader:
                ...
                data_remarketing = excel_date_to_datetime(
                    row.get("data_remarketing", "")
                )
                agendamento = row.get("agendamento", "").strip().upper()
                data_agendamento = excel_date_to_datetime(
                ...
                ultima_interacao = excel_date_to_datetime(
                    row.get("ultima_interacao", "")
                )
                ...
                if email:
                    cur.execute(
                        "SELECT id FROM customers WHERE master_email = ?", (email,)
                    )
                    res = cur.fetchone()
                    if res:
                        master_id = res["id"]
                ...
                if not master_id and whatsapp:
                    cur.execute(
                        "SELECT id FROM customers WHERE master_phone = ?", (whatsapp,)
                    )
                    res = cur.fetchone()
                    if res:
                        master_id = res["id"]
            ...
            conn.commit()
            print(
                f"Import complete! Successfully merged {success_count} valid contacts into the Master DB."
            )
